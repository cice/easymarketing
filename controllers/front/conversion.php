...
class EmarketingConversionModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        $message .= "\r\n".'Request *'.$_SERVER['REQUEST_METHOD'].'*: '.print_r($input_put_data, true);
        ...
        if (isset($_SERVER['HTTP_AUTHORIZATION']) && $_SERVER['HTTP_AUTHORIZATION'] != '' && $_SERVER['HTTP_AUTHORIZATION'] == Configuration::get(Emarketing::$conf_prefix.'SHOP_TOKEN')) {
            ...
            if ($data == null || !isset($data['snippent'])) {
                $headers[] = $_SERVER['SERVER_PROTOCOL'] . ' 400 Bad Request';
            } else {
                // do something / save
                Configuration::updateValue(Emarketing::$conf_prefix.'CONVERSION_TRACKER', $input_put_data);
